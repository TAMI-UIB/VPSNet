...
from upsampling.bicubic import UpsamplingConv, DownsamplingConv, Downsampling
from model.proxnet import ProxNet
from src.utils.losses import RadiometricLoss

class VPSNetConcatMemory(nn.Module):
    ...
    def __init__(self, n_channels: int, sampling_factor: int, **kwargs) -> None:
        ...
        self.proxnet = ProxNet(2*(self.n_channels+self.memory_size), self.n_channels+self.memory_size, self.n_resblocks)
    ...
    def forward(self, ms: Tensor, lms: Tensor, pan: Tensor):
        ...
        for _ in range(self.n_iters):
            ...
            p_extended = p_extended + self.gamma*u_barra_extended - self.gamma * self.proxnet(torch.concat((p_extended/self.gamma, u_barra_extended), dim = -3))
        ...
        result = self.postprocessing(u_extended[:,:self.n_channels,:,:])
...
if __name__ == '__main__':
    upsampling = UpsamplingConv(12, 4)
    downsampling = DownsamplingConv(12, 4)
    ...
    model = VPSNetConcatMemory(8, 4, **kwargs)
