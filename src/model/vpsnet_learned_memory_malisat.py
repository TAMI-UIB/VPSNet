...
class VPSNetLearnedMemoryMalisat(nn.Module):
    ...
    def __init__(self, n_channels: int, sampling_factor: int, **kwargs) -> None:
        ...
        self.upsampling = UpSamp_4_2(self.n_channels+self.memory_size, sampling_factor=sampling_factor)
        self.downsampling = Downsamp_4_2(self.n_channels+self.memory_size, sampling_factor=sampling_factor)

        self.downsampling_2 = Downsampling(1, 2)
    ...
    def forward(self, ms: Tensor, lms: Tensor, pan: Tensor):
        ...
        pan_d2 = self.downsampling_2(pan)

        f = ms
        f_extended = self.extend_tensor(ms)

        u_tilde = self.BU(f_extended, pan, pan_d2)[:,:self.n_channels,:,:]
        P_tilde = self.BU(self.DB(self.extend_tensor(P), pan, pan_d2), pan, pan_d2)[:,:self.n_channels,:,:]

        u_extended = self.extend_tensor(lms)
        p_extended = self.extend_tensor(lms)
        q_extended = self.DB(u_extended, pan, pan_d2)
        ...
        for _ in range(self.n_iters):
            ...
            q_extended = (q_extended + self.gamma*(self.DB(u_barra_extended, pan, pan_d2)-f_extended)) / den_q
            
            u, u_memory = torch.split(u_extended, self.n_channels, dim=1)
            p = torch.split(p_extended, self.n_channels, dim=1)[0]
            q_up = torch.split(self.BU(q_extended, pan, pan_d2), self.n_channels, dim=1)[0]
            ...
            if self.metric_per_stage:
                stages.append(u_extended[:,:self.n_channels,:,:])
...
if __name__ == '__main__':
    ...
    model = VPSNetLearnedMemoryMalisat(n_channels, 4, **kwargs)
