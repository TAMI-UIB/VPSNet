...
from upsampling.upsamplings import UpSamp_4_2, Downsamp_4_2, Upsampling, Downsampling
from model.proxnet import ProxNet


class VPSNetLearnedMalisatFT(nn.Module):
    ...
    def __init__(self, n_channels: int, sampling_factor: int, **kwargs) -> None:
        ...
        self.blurr = DobleConv(self.n_channels, kwargs.get("n_features", 32), self.n_channels).requires_grad_(False)

        self.upsampling = UpSamp_4_2(self.n_channels, sampling_factor=sampling_factor).requires_grad_(False)
        self.downsampling = Downsamp_4_2(self.n_channels, sampling_factor=sampling_factor).requires_grad_(False)
        ...
        self.proxnet = ProxNet(self.n_channels, self.n_channels, self.n_resblocks).requires_grad_(False)
                        
        # Variational parameters
        self.gamma = nn.Parameter(torch.Tensor([1]), requires_grad=False)
        self.tau = nn.Parameter(torch.Tensor([0.05]), requires_grad=False)
        self.lmb = nn.Parameter(torch.Tensor([50]), requires_grad=False)
        self.mu = nn.Parameter(torch.Tensor([10]), requires_grad=False)
    ...
    def freeze_model(self):
        ...
        for param in self.blurr.parameters():
            param.requires_grad = False

        for param in self.proxnet.parameters():
            param.requires_grad = False

        self.gamma.requires_grad = False
        self.tau.requires_grad = False
        self.lmb.requires_grad = False
...
if __name__ == '__main__':
    ...
    model = VPSNetLearnedMalisatFT(n_channels, 4, **kwargs)
