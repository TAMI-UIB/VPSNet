...
from src.upsampling.bicubic import UpSamp_4_2, Downsamp_4_2, Upsampling, Downsampling
from model.proxnet import ProxNet
from src.utils.losses import RadiometricLoss

class VPSNetLearnedMalisatRadiometric(nn.Module):
    ...
    def forward(self, ms: Tensor, lms: Tensor, pan: Tensor):
        ...
        for _ in range(self.n_iters):
            ...
            stages.append(u)
            radiometrics.append(self.radiometric(u, P, P_tilde, u_tilde))

            u_barra = 2*u-u_anterior
        
        result = self.postprocessing(u) if self.postprocessing_type is not None else u

        # El output depende de la loss que usemos
        if self.metric_per_stage:
            # Si usamos métrica por stages y radiometrica por stages, devolvemos las tres
            return (result, stages, radiometrics)

        return result, radiometrics
...
if __name__ == '__main__':
    ...
    kwargs = {
        ...
        "postprocessing_type": 'spectralBlocks',
        "spectral_blocks": 4,
        "use_features": False
        }
    
    model = VPSNetLearnedMalisatRadiometric(n_channels, 4, **kwargs)
